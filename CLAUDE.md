...
- **The default snapshot is lean** (FWL-044): decorative images, footer regions and non-price body text are dropped and unnamed runs folded, because a mid-tier agent does not ask for slimmer output by itself. `raw: true` is the escape hatch, and refs are identical in both modes.
