...
- **A login provider's cookies are stored on their own and injected into every session** (FWL-070). A stored login holds only its site's registrable domain, so a sign-in through Google, Naver, Kakao or Apple was thrown away at `session_end` and a site joined through a provider re-authenticated on every run — and met the provider's automation refusal or second factor every time. Cookies on the four allow-listed provider domains now go to `idp-<domain>.dpapi`, under the same DPAPI-plus-passphrase lock as everything else (rule 11) and the same shrink guard (FWL-025), and are merged into every session's context, which is what one browser profile does. This is a deliberate raise in what is at stake, not a new leak path: the artifact is a provider account session rather than a shop login, and it was accepted for a single-operator home server (2026-09-14). Provider files are not sites — `hasStorageState` and keepalive skip them. Any third-party cookie outside the allow list is dropped as before.
