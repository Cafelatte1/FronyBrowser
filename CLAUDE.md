...
- **An origin is pinned to the launch profile that logged in** (FWL-065). The caller picks browser and headless (FWL-055) and gets it wrong silently: a bot manager blocks by serving an ordinary page, so `session_begin` succeeds, every action succeeds and the audit log shows a normal session — measured on the live log, all three blocked headless sessions on one site ended `normal`, while the combination that worked reached `loggedIn=true` nine times and the blocked ones zero times out of three. So "the session ran" teaches nothing and `session_end(loggedIn=true)` is the only available evidence; it is also the assertion FWL-026 already trusts to overwrite stored cookies, so this adds no new trust. A remembered origin then fills in an omitted profile and rejects a differing explicit one with `profile_mismatch` rather than opening a session the site will block. Two limits make it safe: only a profile the caller **explicitly chose** is learned, because a default nobody picked proves nothing — keepalive omits the profile and passes "the home page loaded" as `loggedIn`, and it would otherwise pin every origin to the default — and a remembered profile that cannot launch is forgotten, so an uninstalled Chrome does not lock the origin out forever. Any other correction is the operator deleting a line from `origins.json`.
