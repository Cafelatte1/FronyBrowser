...
Non-negotiable rules, in short: input values are never serialized; the handler folder is the feature whitelist (no `evaluate`, HTML dump, console or network access); every response passes `egress.ts`; the frame origin is read from the browser for the audit record, never from an argument; nothing that could hold a value reaches the audit log. The full list with rationale is in [docs/architecture.md](docs/architecture.md). Check any new feature against it before writing code.
...
`docs/INDEX.md` lists every doc with when to read it. Read the matching doc before changing the vault or session storage, a handler or MCP tool, an HTTP route, auth, deploy, or the audit log, and update it in the same branch.
