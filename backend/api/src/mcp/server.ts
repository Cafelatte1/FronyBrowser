...
Responses: every tool returns JSON, either { "ok": true, ... } or { "ok": false, "error": { "code", "message", "retriable" } }. "retriable": true (stale_ref, element_not_actionable, session_limit, lease_conflict, navigation_failed, timeout) means the same call may succeed later; after stale_ref take a new page_tree first. These codes need a human and must not be retried — report the code to the user and stop: vault_locked (the operator runs "vault unlock"), keypad_unresolved (the secure keypad's markup changed), grant_required / grant_invalid (the pay grant is missing, expired, already used, or for another session), key_held (the operator held this key back from test runs).

Values never come back: page_tree omits input values, fill responses carry only the key name and length, and any vault value a page echoes is replaced by [REDACTED:key]. Two things sit outside that rule on purpose. First, a key the operator marked public: vault_list returns its value and pages are left alone where they show it, which is how you learn which card is registered or which country the passport is for, so you can pick the matching option on a form — every key without that mark stays hidden from you. Second, page_image is a picture, and that matters: the input fields themselves are masked in it, but a value the site prints on the page — a delivery name, phone or address — is in the image exactly as it looks on screen, so treat an image as carrying personal data even though the text around it does not.
...
export function buildMcpServer(deps: McpDeps, caller: Caller): McpServer {
  ...
  server.registerTool(
    ...
    {
      description:
        'Open a browser session bound to one exact origin (session = one origin). The origin\'s exclusive lease and stored login session are fixed here; browser/headless are chosen by you and fixed for the session; omit both and an origin that has logged in before reopens the way it worked. Your navigate calls must stay inside this origin (redirects the page performs itself, e.g. to a payment or login provider, are allowed). The response carries storedLogin: true when a stored login context was injected (it may have expired — verify on the page), false when this origin has none yet (go straight to the login page and use the vault login keys); neither is an error. vault_locked needs a human (operator runs "vault unlock") — report and stop.',
      inputSchema: {
  ...
  server.registerTool(
    'page_tree',
    {
      ...
      inputSchema: {
        ...
        after: ref.optional().describe('Continue a cut tree from just after this ref — the ref named by the previous page_tree\'s "… more lines did not fit" line.'),
      },
    },
    async ({ sessionId: sid, ref: r, filter, raw, after }) =>
      out('page_tree', await deps.handlers.page_tree(caller, sid as SessionId, {
        ...(r === undefined ? {} : { ref: r as Ref }),
        ...(filter === undefined ? {} : { filter }),
        ...(raw === undefined ? {} : { raw }),
        ...(after === undefined ? {} : { after: after as Ref }),
      })),
  );
  ...
  server.registerTool(
    ...
    {
      description:
        'Type into a field as real keystrokes (the site sees keydown, input and keyup for every character, so its own validation runs). A field shown [disabled] in page_tree cannot be filled: finish the step that enables it and take a new page_tree. Put "{{vault:key}}" placeholders in the value; the server substitutes the real value, which never appears in any response. Available keys: vault_list. For a secure keypad (payment PINs) pass keypad and point ref at any element inside the keypad frame, e.g. its heading — the server presses the digit keys itself and reports keypad_unresolved if it cannot read the keypad. Never click digit keys yourself. Keys registered with the grant flag additionally need grant.',
      inputSchema: {
        sessionId,
        ref,
  ...
  server.registerTool(
    'vault_list',
    { description: 'Names, types and labels of the keys registered in the vault, each with a grant flag saying whether fill needs a pay grant for it, and a public flag. A public key also carries its value: the operator marked it safe to read, so use it to choose between options a page offers — the card issuer to select, the country on the passport. Every other key returns no value and no length.', inputSchema: {} },
