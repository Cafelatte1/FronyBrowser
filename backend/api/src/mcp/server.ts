...
export function buildMcpServer(deps: McpDeps, caller: Caller): McpServer {
  ...
  server.registerTool(
    ...
    {
      description:
        'Open a browser session bound to one exact origin (session = one origin). The origin\'s exclusive lease and stored login session are fixed here; browser/headless are chosen by you and fixed for the session, unless this origin has logged in before — then the combination that worked wins (see profile_mismatch). Your navigate calls must stay inside this origin (redirects the page performs itself, e.g. to a payment or login provider, are allowed). The response carries storedLogin: true when a stored login context was injected (it may have expired — verify on the page), false when this origin has none yet (go straight to the login page and use the vault login keys); neither is an error. vault_locked needs a human (operator runs "wallet unlock") — report and stop.',
      inputSchema: {
