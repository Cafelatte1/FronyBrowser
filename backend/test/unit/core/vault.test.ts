...
function seed(name: string): string {
  ...
  writeVaultFile(
    ...
    new Map([
      ...
      ['card.number', { type: 'card', value: '1111222233334444', grant: false, public: false, label: 'Card number' }],
    ]),
    fakeCipher,
  );
  return path;
...
describe('vault', () => {
  it('쓰기 → unlock → get/list/live 왕복', async () => {
    ...
    expect(vault.list()).toContainEqual({ name: 'card.number', type: 'card', len: 16, grant: false, public: false, label: 'Card number' });
  ...
  it('applyWrite — 열려 있으면 방금 쓴 항목으로 메모리를 갈아끼운다. 잠긴 금고는 열지 않는다 (FWL-058)', async () => {
    ...
    const written: Map<string, VaultEntry> = new Map([
      ['phone', { type: 'phone', value: '01099998888', grant: true, public: false, label: 'Mobile' }],
    ]);
    ...
    await vault.unlock('correct-horse');
    vault.applyWrite(written);
    expect(vault.list()).toEqual([{ name: 'phone', type: 'phone', len: 11, grant: true, public: false, label: 'Mobile' }]);
  ...
  it('파일 뮤테이션 — set/remove/overview, 첫 set이 패스프레이즈를 확정한다', () => {
    ...
    setVaultEntry(path, 'pp', 'login.shop.com.id', { type: 'text', value: 'me@x.com', grant: false, public: false, label: 'Login ID' }, fakeCipher);
    setVaultEntry(path, 'pp', 'login.shop.com.pw', { type: 'text', value: 'hunter22', grant: false, public: false, label: 'Login password' }, fakeCipher);

    const overview = overviewVaultFile(path, 'pp', fakeCipher);
    expect(overview).toContainEqual({ name: 'login.shop.com.pw', type: 'text', len: 8, grant: false, public: false, label: 'Login password' });
    expect(JSON.stringify(overview)).not.toContain('hunter22'); // 값은 나가지 않는다

    expect(() => setVaultEntry(path, 'wrong', 'x', { type: 'text', value: 'v', grant: false, public: false, label: 'X' }, fakeCipher)).toThrow();
  ...
  it('grant 플래그 — 저장한 대로 돌아오고, 필드가 없는 예전 파일은 false로 읽힌다', async () => {
    ...
    setVaultEntry(path, 'pp', 'shop.payment.pin', { type: 'text', value: 'x', grant: true, public: false, label: 'Pin' }, fakeCipher);
    expect(overviewVaultFile(path, 'pp', fakeCipher)).toContainEqual({
      name: 'shop.payment.pin', type: 'text', len: 1, grant: true, public: false, label: 'Pin',
    });
    ...
    expect(vault.list()).toEqual([{ name: 'phone', type: 'phone', len: 11, grant: false, public: false, label: '' }]);
  });
...
describe('라벨 (FWL-056)', () => {
  ...
  it('seedLabels — 백업을 먼저 만들고 빈 라벨만 채운다. 두 번째 호출은 no-op이고 백업도 더 만들지 않는다', () => {
    ...
    expect(after).toContainEqual({ name: 'profile.personal.phone', type: 'phone', len: 11, grant: false, public: false, label: 'Mobile' });
    // 사용자가 이미 지어 둔 이름은 건드리지 않는다
    expect(after).toContainEqual({ name: 'card.personal.cvv', type: 'text', len: 3, grant: true, public: false, label: '내 카드 뒷자리' });
...
describe('키 이름 이전 (FWL-057)', () => {
  ...
  it('migrateKeyNames — 백업을 먼저 만들고 열 개를 그대로 옮긴다. 두 번째 호출은 no-op이고 백업도 더 만들지 않는다', () => {
    ...
    writeVaultFile(
      path,
      'pp',
      new Map(OLD_KEYS.map((key, i) => [key, { type: 'text' as const, value: 'v'.repeat(i + 1), grant: i % 2 === 0, public: false, label: `L${i}` }])),
      fakeCipher,
    );
    const before = readFileSync(path);
    ...
    expect(overviewVaultFile(path, 'pp', fakeCipher)).toEqual(
      OLD_KEYS.map((key, i) => ({ name: newName(key), type: 'text', len: i + 1, grant: i % 2 === 0, public: false, label: `L${i}` })),
    );
...
describe('공개 플래그 (FWL-080)', () => {
  it('public이 없는 예전 파일은 false로 읽고, true는 그대로 왕복한다', () => {
    const dir2 = mkdtempSync(join(tmpdir(), 'vault-public-'));
    const path = join(dir2, 'v.dpapi');

    // public 칸이 아예 없는 예전 파일 — 복호화된 JSON을 직접 만들어 넣는다
    const legacy = JSON.stringify({ 'card.personal.issuer': { type: 'text', value: '카카오뱅크', grant: false, label: 'Issuer' } });
    writeFileSync(path, fakeCipher.protect(Buffer.from(legacy, 'utf8'), createHash('sha256').update('pp', 'utf8').digest()));
    expect(readVaultFile(path, 'pp', fakeCipher).get('card.personal.issuer')?.public).toBe(false);

    writeVaultFile(path, 'pp', new Map([
      ['card.personal.issuer', { type: 'text' as const, value: '카카오뱅크', grant: false, public: true, label: 'Issuer' }],
    ]), fakeCipher);
    expect(readVaultFile(path, 'pp', fakeCipher).get('card.personal.issuer')?.public).toBe(true);
