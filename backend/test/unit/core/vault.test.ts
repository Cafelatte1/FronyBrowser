...
function seed(name: string): string {
  ...
  writeVaultFile(
    ...
    'correct-horse',
    new Map([
      ['phone', { type: 'phone', value: '01012345678' }],
      ['card.number', { type: 'card', value: '1111222233334444' }],
    ]),
    fakeCipher,
  );
  return path;
...
describe('vault', () => {
  it('쓰기 → unlock → get/list/live 왕복', async () => {
    ...
    expect(vault.list()).toContainEqual({ name: 'card.number', type: 'card' });
  ...
  it('파일 뮤테이션 — set/remove/overview, 첫 set이 패스프레이즈를 확정한다', () => {
    ...
    setVaultEntry(path, 'pp', 'login.shop.com.id', { type: 'text', value: 'me@x.com' }, fakeCipher);
    setVaultEntry(path, 'pp', 'login.shop.com.pw', { type: 'text', value: 'hunter22' }, fakeCipher);

    const overview = overviewVaultFile(path, 'pp', fakeCipher);
    expect(overview).toContainEqual({ name: 'login.shop.com.pw', type: 'text', len: 8 });
    expect(JSON.stringify(overview)).not.toContain('hunter22'); // 값은 나가지 않는다

    expect(() => setVaultEntry(path, 'wrong', 'x', { type: 'text', value: 'v' }, fakeCipher)).toThrow();
